...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def never[T]: Future[T] = {
      val p = Promise[T]()
      p.future // this future will never complete
    }
    /**
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      fs.foldRight(Future.always(List[T]())) { // we use always because we want our accumulator to always complete so flatmap can complete and control can go back to subsequent map operation
        (f, acc) => acc.flatMap { t => f.map { h => h :: t } }
        // or write this as 
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      ...
      f.onComplete {
        case _ =>
          try {
            //p.success(cont(f))
            p.tryCompleteWith(Future(cont(f)))
          } catch {
            case _: Exception => p.failure(_)
          }
      }
      p.future
    }
      ...
      f.onComplete {
        case tryValue =>
          try {
            ...
            p.tryCompleteWith(Future(cont(tryValue)))
          } catch {
            case _: Exception => p.failure(_)
          }
      }
