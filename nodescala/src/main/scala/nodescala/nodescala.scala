...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val requestServerLoop = Future.run() {
      token => async {
        while ( !token.isCancelled ) {
          val (request, exchange) = await { listener.nextRequest() } 
          async { respond(exchange, token, handler( request) ) }
        }
       // here means no more requests from the listener, cancel the subscription
      listenerSubscription.unsubscribe 
      }
    }

    Subscription(listenerSubscription, requestServerLoop) // composite subscription returned so when either cancels both listener and server stop
  }
